...
function Arraignment() {
  ...
  const addCrime = () => {
    setFormData((prev) => ({ ...prev, counts: [...prev.counts, { crime: "" }] }));
  };
  ...
  const removeCrime = (index: number) => removeItem("counts", index);
  ...
  return (
    <div className="tpl-page">
          ...
          <div key={index} className="tpl-group">
            <div className="tpl-row">
              ...
              {formData.counts.length > 1 && (
                <button className="tpl-btn-remove" onClick={() => removeCrime(index)} title="Remove">−</button>
              )}
            </div>
          </div>
        ))}
        <button className="tpl-btn-add" onClick={addCrime}>+ Add Crime</button>
      </div>
      ...
      <div className="tpl-section">
        ...
        <div className="tpl-option-group" style={{ marginTop: "1rem" }}>
          ...
          <label className="tpl-check">
            <input type="radio" name="exhibitPattern" value="numbers" checked={exhibitPattern === "numbers"} onChange={handleExhibitPatternChange} />
            Numbers (1, 2, 3…)
          </label>
          <label className="tpl-check">
            <input type="radio" name="exhibitPattern" value="letters" checked={exhibitPattern === "letters"} onChange={handleExhibitPatternChange} />
            Letters (A, B, C…)
          </label>
        </div>
      </div>
      ...
      <div className="tpl-section">
        <span className="tpl-section-title">Sentencing Options</span>
      ...
      <div className="tpl-section">
        ...
        <div className="tpl-option-group">
          ...
          {formData.bail.option === "imposeBail" && (
            <div className="tpl-indent">
              ...
              <label className="tpl-check"><input type="checkbox" name="bail.abstainAlcohol" checked={formData.bail.abstainAlcohol} onChange={handleChange} />Abstain from excessive use of alcohol and any use of a controlled substance without a prescription.</label>
              <label className="tpl-check"><input type="checkbox" name="bail.appearCourt" checked={formData.bail.appearCourt} onChange={handleChange} />Appear before the court on each date the case is assigned to call.</label>
              <label className="tpl-check"><input type="checkbox" name="bail.abstainCriminalActivity" checked={formData.bail.abstainCriminalActivity} onChange={handleChange} />Abstain from criminal activity while subject to the bail order.</label>
              ...
              <label className="tpl-check"><input type="checkbox" name="bail.surrenderFirearms" checked={formData.bail.surrenderFirearms} onChange={handleChange} />Surrender any illegal or legally owned firearms to the respective agency.</label>
              <label className="tpl-check"><input type="checkbox" name="bail.possessCellPhone" checked={formData.bail.possessCellPhone} onChange={handleChange} />Possess and keep on at all times an active cell phone with its number registered to Defendant.</label>
              <label className="tpl-check"><input type="checkbox" name="bail.refrainContact" checked={formData.bail.refrainContact} onChange={handleChange} />Refrain from contact with any victim/witness relating to this arrest.</label>
        ...
        <div className="tpl-option-group">
          ...
          {formData.bail.option === "orderDetention" && (
            <div className="tpl-indent">
              <label className="tpl-check"><input type="checkbox" name="bail.felonyViolence" checked={formData.bail.felonyViolence} onChange={handleChange} />The defendant is charged with a felony crime of violence and has over 30 criminal points on record.</label>
              <label className="tpl-check"><input type="checkbox" name="bail.maxSentence" checked={formData.bail.maxSentence} onChange={handleChange} />The defendant is charged with an offense for which the maximum sentence is life imprisonment or death.</label>
              <label className="tpl-check"><input type="checkbox" name="bail.historyViolatingBail" checked={formData.bail.historyViolatingBail} onChange={handleChange} />The defendant has a history of violating bail conditions.</label>
              <label className="tpl-check"><input type="checkbox" name="bail.threatenWitness" checked={formData.bail.threatenWitness} onChange={handleChange} />There exists a serious risk that the defendant will threaten, injure, or intimidate a prospective witness.</label>
              <label className="tpl-check"><input type="checkbox" name="bail.noBailConditions" checked={formData.bail.noBailConditions} onChange={handleChange} />The People believe that no condition or combination of conditions will reasonably assure the defendant's appearance as required and the safety of any person or the community.</label>
